...
$logFilePath = "C:\TempLIS\Termination Logs\$logFileName"
...
WHERE p.adlogin IS NOT NULL
	...
	AND TRY_CONVERT(date, t.lastdt, 105) = CONVERT(date, DATEADD(day, +2, GETDATE()))
...
foreach ($row in $sqlData) 
{
	...
	if ($userID -in 'lasbes0', 'lasdol2', 'lasjeb0') 
	{
    ...
    $EmpName = $row.empname
	$MemberGroups=$null
	$targetOU = "OU=$year,OU=Resigned Users,OU=LAS,DC=las,DC=domain,DC=i"
		...
		try 
		{
			...
			$MemberGroups=$userGroups -join ', '
			 # Check if specific group exists in user's memberships
              $VisioIsInGroup = $userGroups -contains ".LAS-M365-License-Visio-P2"
			  $ProjectIsInGroup = $userGroups -contains ".LAS-M365-License-Project-P3"
...
#			 $user | Set-ADUser -Replace @{Description = $Description}		
       ...
       try {
		...
		try
		{
        $folderPath = "C:\TempLIS\Termination\$EmpName"
         ...
         $outputExcelFilePath = "C:\TempLIS\Termination\$EmpName\TitleOutputFile_$timestamp.xlsx"
...
$EmailTo = "b.a@gmail.com"  # Recipient's email address     
$EmailSubject = "$empName has been Terminated $LastDay"
#$EmailCc="c.d@gmail.com"
...
#To create an excel file Date Wise to find out what IDs were deleted on which Date
	   ...
	   {             
              $timestamp = Get-Date -Format "dd-MM-yyyy hhmmss"
            # Specify the path to the output Excel file with timestamp
             $outputExcelFilePath = "C:\TempLIS\Termination\TitleOutputFile_$timestamp.xlsx"
             # Export the output data to a new Excel file
            $outputData | Export-Excel -Path $outputExcelFilePath -AutoSize
			
					
			#Add a log for Excel file creation  
	         "Success: Excel file for all the IDS has been created successfully for today" | Out-File -FilePath $logFilePath -Append
